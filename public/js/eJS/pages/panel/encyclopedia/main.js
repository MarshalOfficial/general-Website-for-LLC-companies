...
function DeleteArt(nodeId)
{
	...
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.article.deleteArt%>',
		type: 'POST',
		data:
		{
			nodeId: nodeId
...
function CheckURLName(item)
{
	$.ajax
	({
		url: '/' + lang + '<%=consV.pages.panel.article.URLNameValidation%>',
		type: 'POST',
		data:
		{
			nodeId: item._id,
		...
		{			
			...
			else if( res == "<%=consV.codes.db.docFound%>" || res == "<%=consV.codes.validationError%>")
			{
